"""Detect private state, credentials, and restricted artifacts in a Git index."""
...
class GitIndexError(RuntimeError):
    """Raised when the index cannot be inspected, so the guard cannot fail open."""
...
def find_forbidden_tracked_files(
    ...
    """Judge paths and staged text with no filesystem or Git access of its own.

    Keeping the decision pure is what makes every rule testable without a
    repository, and it leaves the impure half with nothing to get wrong beyond
    reading the index. A path with no decodable text is still judged by its
    name, which is what catches model weights.
    """
...
def check_repository(repo_root: Path) -> tuple[str, ...]:
    """Read tracked paths and their staged bytes from Git, then apply the pure guard."""

    if not repo_root.is_dir():
        raise GitIndexError(f"not a directory: {repo_root}")
    ...
    return find_forbidden_tracked_files(tracked_paths, tracked_text)
