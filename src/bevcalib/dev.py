...
def _stage_commands() -> dict[str, tuple[str, ...]]:
    ...
    return {
        "private_guard": (python, "-m", "bevcalib.private_guard"),
...
def verify_repository(repo_root: Path, runner: StageRunner = subprocess_runner) -> int:
    """Run every stage in fixed order, stopping at the first non-zero exit.

    Stopping early is the point. A later gate's output buries the earliest
    cause, and every stage after a real failure spends minutes telling you
...
def verify_schema_contracts(repo_root: Path) -> int:
    """Parse every JSON schema and report all malformed files, not just the first."""
...
def verify_docs_links(repo_root: Path) -> int:
    ...
    for source, missing_target in broken:
        ...
        print(f"broken local link: {relative_source} -> {missing_target}", file=sys.stderr)
