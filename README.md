...
- **Evidence 與圖表：** `docs/evidence/`、claims registry `docs/claims.yaml`、`docs/figures/`、`docs/analysis/`，以及它們在網站上的副本與表格，都是由 nuScenes v1.0-trainval 衍生的彙總量測。它們不含影像、點雲、sample token 或場景名稱，依 nuScenes 使用條款（CC BY-NC-SA 4.0）供非商業研究使用。nuScenes 本身由 Motional 散布，不在此轉散布。使用這些結果時請引用 nuScenes（Caesar et al., CVPR 2020），引用資訊見 [NOTICE](NOTICE)。
